...
class DeepNeuralNetwork:
    ...
    def gradient_descent(self, Y, cache, alpha=0.05):
        """returns the gradient descent"""
        m = Y.shape[1]
        prev = {}
        for i in range(self.L, 0, -1):
            A = cache["A{}".format(i - 1)]
            if i == self.L:
                prev["dz{}".format(self.L)] = cache["A{}".format(self.L)] - Y
            else:
                prev_dz = prev["dz{}".format(i + 1)]
                currA = cache["A{}".format(i)]
                prev["dz{}".format(i)] = np.matmul(
                    prevW.transpose(), prev_dz) * (currA * (1 - currA))
            dz = prev["dz{}".format(i)]
            dw = (1 / m) * np.matmul(dz, A.transpose())
            db = (1 / m) * np.sum(dz, axis=1, keepdims=True)
            prevW = self.weights["W{}".format(i)]
            self.__weights["W{}".format(
                i)] = self.weights["W{}".format(i)] - (alpha * dw)
            self.__weights["b{}".format(
                i)] = self.weights["b{}".format(i)] - (alpha * db)
